...
amwaysearch.components.search = (function($, window) {
    ...
    var updateFlow = function(url) {
        var loadAjax = $.ajax({
            dataType: 'json',
            url: '/bin/search/solr_update?action=updateFlow&content=' + encodeURIComponent(url)
        });
    ...
    var redirectUrl = function(url, contentTagSource, sourceTag) {
        if (contentTagSource && sourceTag) {
            var sourceTagTrack = sourceTag+ '-' + contentTagSource ;
            /*global s */
            s.linkTrackVars = 'prop12';
            s.prop12 = sourceTagTrack;
            s.useForcedLinkTracking = false;
            s.tl(true, 'o', sourceTagTrack+' （内容属性标签）');
        }
        window.location.href = url;
    };
    var searchByHotword = function(hotWord, source, hasTags, redirectLink, searchWord){
        ...
        if(hotWord !== '' && source !== ''){
            ...
            s.prop11 = hotWord + '-' + source;
            s.useForcedLinkTracking = false;
            s.tl(true, 'o', hotWord+' （'+ source +'热词）');
        }
        if (redirectLink !== '') {
    ...
    /* jshint maxlen: 120 */
     var searchByCategory = function(categoryType, category, categoryTitle, hasChild, redirectLink) {
        if(categoryType !== '' && categoryTitle !== ''){
            s.linkTrackVars = 'prop10';
            s.prop10 = window.yunzhidao.analytics.category[categoryType] + categoryTitle;
            s.useForcedLinkTracking = false;
            s.tl(true, 'o', window.yunzhidao.analytics.category[categoryType] + categoryTitle +' （热门分类）');
        }
        if (redirectLink !== '') {
            window.location.href = redirectLink;
            return false;
